...
class Diffusion(L.LightningModule):
  ...
  def _forward_pass_diffusion(self, x0, t=None, sampling_eps_min=None, sampling_eps_max=None):
    ...
    # In ppl_eval mode, optionally use entropy as a proxy to select between two candidates for xt (without using ground truth)
    # Controlled by config.sampling.entropy_guided_xt, defaults to False to ensure original behavior is preserved
    use_entropy_guided_xt = getattr(self.config.sampling, 'entropy_guided_xt', False)
    # print('use_entropy_guided_xt', use_entropy_guided_xt)
    if use_entropy_guided_xt and (not self.training) and self.config.mode == 'ppl_eval':
      # Only enable in subs parameterization to avoid affecting other modes
      # print("samling ...")
      # print(self.parameterization, self.cross_attn)
      if (self.parameterization == 'subs'): # and (not self.cross_attn):
        with torch.no_grad():
          # print("using entropy guidance for xt sampling ...")
          # Generate a second candidate xt_alt: resample q_xt
          xt_alt = self.q_xt(x0,
                              p,
                              sampling_eps_min=sampling_eps_min,
                              sampling_eps_max=sampling_eps_max)
          # Calculate "prediction entropy" for both candidates under model's predictive distribution: lower is better (more confident)
          # Note: This does not use x0 labels at all, only looks at model's own output
          sigma_detached = self._sigma_from_p(p[:,0].unsqueeze(-1))
          
          # Construct input in the same way as formal forward (considering cross_attn and ignore_bos)
          xt_for_forward = xt.clone()
          xt_alt_for_forward = xt_alt.clone()
          if self.ignore_bos:
            xt_for_forward[:, 0] = x0[:, 0]
            xt_alt_for_forward[:, 0] = x0[:, 0]
          
          # If cross_attn is used, need to concatenate x0
          if self.cross_attn:
            x_input_1 = torch.cat((xt_for_forward, x0), dim=-1)
            x_input_2 = torch.cat((xt_alt_for_forward, x0), dim=-1)
          else:
            x_input_1 = xt_for_forward
            x_input_2 = xt_alt_for_forward
          
          # Forward pass for each candidate
          log_score_xt = self.forward(x_input_1, sigma=sigma_detached)
          log_score_xt_alt = self.forward(x_input_2, sigma=sigma_detached)
          
          # Probability distributions
          prob_xt = log_score_xt.exp()
          prob_xt_alt = log_score_xt_alt.exp()
          # Token-level entropy along the vocab dimension
          entropy_xt = - (prob_xt * log_score_xt).sum(dim=-1)  # (B, L)
          entropy_xt_alt = - (prob_xt_alt * log_score_xt_alt).sum(dim=-1)
          # For each sample, compare the average entropy over the whole sequence, select the path with lower entropy
          mean_entropy_xt = entropy_xt.mean(dim=-1)         # (B,)
          mean_entropy_xt_alt = entropy_xt_alt.mean(dim=-1) # (B,)
          better_alt = mean_entropy_xt_alt < mean_entropy_xt
          print(f"[DEBUG] Better alt: {better_alt.sum().item()}/{better_alt.shape[0]}, mean_ent_xt={mean_entropy_xt.mean().item():.3f}, mean_ent_alt={mean_entropy_xt_alt.mean().item():.3f}")
          # Where better_alt is true, replace xt with xt_alt
          if better_alt.any():
            # Broadcast to token dimension
            better_alt_tokens = better_alt[:, None].expand_as(xt)
            xt = torch.where(better_alt_tokens, xt_alt, xt)
  ...
  def _check_stop_conds(self, x):
    ...
    # entropy-based stopping: allow turning it off or changing threshold via config
    last_block = x[:, -256:]
    entropy = self._compute_entropy(last_block)
    # use configurable threshold if provided, else default to 4
    entropy_thresh = getattr(self.config.sampling, 'entropy_threshold', 4.0)
    use_entropy_stop = getattr(self.config.sampling, 'use_entropy_stop', True)
    if use_entropy_stop and entropy < entropy_thresh:
      stop = True
